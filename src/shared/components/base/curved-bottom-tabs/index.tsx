...
const FloatingButtonComponent: React.FC<FloatingButtonComponentProps> =
  memo<FloatingButtonComponentProps>(
    ({ icon, gradient, scale, shadow, badge }: FloatingButtonComponentProps) => {
...
const BackgroundCurve: React.FC<BackgroundCurveProps> = memo<BackgroundCurveProps>(
...
const CurvedBottomTabsCore: React.FC<CurvedBottomTabsProps> = memo<CurvedBottomTabsProps>(
  ({
    tabs,
    currentIndex,
    onPress,
    ...
    const animateToIndex = <T extends number>(targetIndex: T): void => {
      const targetPosition = calculateTabPosition<number, number>(targetIndex, tabs.length);
      ...
      floatingAnimations.forEach((anim: SharedValue<number>, index: number) => {
        anim.value = withSpring<number>(index === targetIndex ? -VIEWPORT_HEIGHT * 4.2 : 0, {
          damping: 10,
          stiffness: 100,
          mass: 0.5,
        });
...
const createStyles = <T extends StyleConfig>({ barHeight, textSize, fontFamily, ...props }: T) =>
  StyleSheet.create({
    wrapper: {
      position: "absolute",
      bottom: 0,
...
export const CurvedBottomTabs: React.FC<BottomTabBarProps & CurvedTabBarNavigationProps> &
  React.FunctionComponent<BottomTabBarProps & CurvedTabBarNavigationProps> = memo(
  ({ state, descriptors, navigation, gradients = ["#121212", "#1A1A1A"] }) => {
