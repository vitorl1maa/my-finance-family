...
export function AuthScreen({ children }: PropsWithChildren) {
  return <ScrollView contentContainerStyle={styles.screen} contentInsetAdjustmentBehavior="automatic">{children}</ScrollView>;
}
...
export function PrimaryButton({ children, onPress, disabled }: { children: ReactNode; onPress: () => void; disabled?: boolean }) {
  return <Pressable accessibilityRole="button" disabled={disabled} onPress={onPress} style={[styles.primaryButton, disabled && styles.disabled]}><Text style={styles.primaryText}>{children}</Text></Pressable>;
}

export function SocialButton({ icon, children, onPress }: { icon: string; children: ReactNode; onPress: () => void }) {
  return <Pressable accessibilityRole="button" onPress={onPress} style={styles.socialButton}><Text style={styles.socialIcon}>{icon}</Text><Text style={styles.socialText}>{children}</Text></Pressable>;
}
...
export function FormError({ message }: { message: string | null }) {
  return message ? <View style={styles.formError}><Text style={styles.errorText}>{message}</Text></View> : null;
}

export function LoadingLabel({ isLoading, label }: { isLoading: boolean; label: string }) {
  return isLoading ? <ActivityIndicator color={colors.text} /> : <Text style={styles.primaryText}>{label}</Text>;
}
...
const styles = StyleSheet.create({
  ...
  eyebrow: { color: colors.muted, fontSize: 13, fontWeight: '800', letterSpacing: 1.1, textTransform: 'uppercase' },
  ...
  socialButton: { alignItems: 'center', borderColor: colors.border, borderRadius: 14, borderWidth: 1, flexDirection: 'row', gap: 12, justifyContent: 'center', minHeight: 52 },
